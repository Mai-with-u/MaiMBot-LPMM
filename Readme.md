...
LPMM模块基于`HippoRAG 2`构建，使用`RAG`模型进行知识图谱的构建与检索。`HippoRAG 2`是一个基于`RAG`的知识图谱系统，支持离线增量式学习、在线智能检索等功能。
...
收到一条检索请求后，LPMM会对请求进行处理，具体如下：
 ...
 - **文本预处理**：对检索请求的文本进行Embedding，转化为向量；
 - **关系检索**：使用`Faiss`对RelationEmbedding库进行检索，找到与检索请求最相似的`Top K`个关系描述；
 - **LLM过滤关系检索结果**：使用LLM对检索结果进行过滤，保留对解决问题有帮助的Fact；
 - **检查是否有相关的关系检索结果**
   - 如果有，则对检索结果进行图搜索与PageRank，获取最终的检索结果；
   - 如果没有，则对知识库中保有的具体文段进行密集段落检索（DPR），获取最终的检索结果；
 - **返回检索结果**：根据上一步中的检索结果，获取相应的文段，返回给用户。
